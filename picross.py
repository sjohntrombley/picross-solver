...
def solve(p):
    if type(p) is not Puzzle:
        raise TypeError("First argument to solve must be of Puzzle type.")
...
def main():
    ...
    while n_rows==0:
        ...
        if len(dimensions)==2:
            try:
                ...
                n_row, n_cols = 0, 0
                print(
                        'Error: Invalid dimensions. Dimensions should be ' +
                        'input in the form rxc where are is the number of ' +
                        'rows and c is the number of columns.'
                        )
        else:
            print(
                    'Error: Invalid dimensions. Dimensions should be input ' +
                    'in the form rxc where are is the number of rows and c ' +
    ...
    for row_index in range(n_rows):
        ...
        while not valid_clue:
            try:
                row_clues.append([
                    int(clue) for clue in
                    input(f'Enter clues for row {row_index+1}: ').split(' ')
                    ])
                valid_clue = True
            except ValueError:
                print(
                        'Error: Invalid clue. Clues should be a list of ' +
    ...
    for col_index in range(n_cols):
        ...
        while not valid_clue:
            try:
                col_clues.append([
                    int(clue) for clue in
                    input(f'Enter clues for col {col_index+1}: ').split(' ')
                    ])
                valid_clue = True
            except ValueError:
                print(
